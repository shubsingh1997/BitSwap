...
router.get('/daily-clogs', function (req, res) {
  ...
  var sql = `SELECT * FROM mydb.Transaction T, mydb.Transaction_Execution E where T.Transaction_ID = E.Transaction_ID and E.Trader_ID IS NULL;`;
...
router.get('/daily-tlogs', function (req, res) {
  ...
  var sql = `SELECT * FROM mydb.Transaction T, mydb.Transaction_Execution E where T.Transaction_ID = E.Transaction_ID and E.Trader_ID != '';`;
  db.query(sql, function (err, results) {
    if (err) throw err;
...
router.get('/total-clients', function (req, res) {
  var sql = "SELECT TE.Client_ID FROM mydb.Transaction T,mydb.Transaction_Execution TE where T.Transaction_ID = TE.Transaction_ID group by(TE.Client_ID);";
  db.query(sql, function (err, results) {
    if (err) throw err;
    res.render('statistics', { Clients: results });
  });
});

router.get('/total-traders', function (req, res) {
  var sql = "SELECT TE.Trader_ID FROM mydb.Transaction T,mydb.Transaction_Execution TE where T.Transaction_ID = TE.Transaction_ID group by(TE.Trader_ID); ";
  db.query(sql, function (err, results) {
    if (err) throw err;
    res.render('statistics', { Traders: results });
  });
});

router.get('/total-dCommision', function (req, res) {
  var sql = "Select sum(Commision_Paid) as commision from Transaction where Commision_type = 'Dollars';";
  db.query(sql, function (err, results) {
    if (err) throw err;
    res.render('statistics', { dollarCommision: results[0] });
  });
});

router.get('/total-bCommision', function (req, res) {
  var sql = "Select sum(Commision_Paid) as commision from Transaction where Commision_type = 'Bitcoins'; ";
  db.query(sql, function (err, results) {
    if (err) throw err;
    res.render('statistics', { BitcoinCommision: results[0] });
  });
});
